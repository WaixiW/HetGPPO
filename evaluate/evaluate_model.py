...
def compute_action(
    ...
    for i, val in enumerate([vel_x, vel_y, pos_x, pos_y]):
        observation[..., i] = val
    logits = model(observation, None, None)[0].detach()
    action_dist = TorchDiagGaussian(logits[0], u_range)
    if deterministic:
...
def compute_action_het_goal(
    ...
    observation = torch.zeros((1, n_agents, obs_size))
    for i, val in enumerate([vel_x, vel_y, pos_x - goal_x, pos_y - goal_y]):
        observation[..., i] = val
    logits = model(observation, None, None)[0].detach()
    action_dist = TorchDiagGaussian(logits[0], u_range)
    if deterministic:
...
if __name__ == "__main__":
    checkpoint_path = "/Users/Matteo/Downloads/MultiPPOTrainer_het_goal_7bdf7_00000_0_2022-09-26_13-05-16/checkpoint_000164/model_a_range_1_u_range_1_circular_control_[3_3_0_002].pt"
    model = torch.load(checkpoint_path)
    model.eval()
    print(
        compute_action_het_goal(
            pos_x=0,
            pos_y=0,
            vel_x=0,
            vel_y=0,
