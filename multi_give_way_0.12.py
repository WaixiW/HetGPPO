...
class Scenario(BaseScenario):
    def make_world(self, batch_dim: int, device: torch.device, **kwargs):
        ...
        self.final_reward = kwargs.get("final_reward", 1.5)
        # self.energy_reward_coeff = kwargs.get("energy_rew_coeff", 0)

        self.agent_collision_penalty = kwargs.get("agent_collisiZon_penalty", -0.1)
                ...
                agent.set_pos(
                    torch.tensor(
                        [
                            ...
                            * (-1 if i == 0 else 1),
                            (self.agent_box_width-self.goal_dist_from_wall)
                            * (-1 if i == 0 else 1),
                        ],
                        dtype=torch.float32,
